...
export async function generateStaticParams() {
  try {
    ...
    console.log(`generateStaticParams for categories: Found ${allSlugs.length} category slugs`);
    return allSlugs.map((slug) => ({
      slug: slug,
    }));
  } catch (error) {
    console.error('Error in category generateStaticParams:', error);
    return [];
  }
}
    ...
    <section data-bs-version="5.1" className="content2 cid-socNq9ZEoK mbr-bg-black" id="content2-q">
      <div className="container">
        <div className="mbr-section-head">
          <h4 className="mbr-section-title mbr-fonts-style align-center mb-0 display-2 mbr-white">
            <strong>{category.icon} {category.name}</strong>
          </h4>
          <h5 className="mbr-section-subtitle mbr-fonts-style align-center mb-0 mt-2 display-5 mbr-white">
            {category.description}
          </h5>
          <p className="mbr-text mbr-fonts-style align-center mt-2 display-7 mbr-white">
